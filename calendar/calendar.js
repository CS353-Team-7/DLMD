...
// waterNeeded = (plantData.data.main_species.growth.minimum_precipitation.mm + plantData.data.main_species.growth.maximum_precipitation.mm) / 2;
...
document.getElementById("search_for_plant").addEventListener("submit", (event) => {
    ...
    */
    ...
    for(const property in plantData.data) {
        ...
        $(`<h1>${plantData.data[property].common_name}</h1>`).appendTo($(`.result-container-${index}`));
        $(`<h4>${plantData.data[property].scientific_name}</h4>`).appendTo($(`.result-container-${index}`));
